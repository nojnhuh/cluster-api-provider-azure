...
type FakeClient struct {
	...
	// server-side apply until v0.22, so we make our own dollar store version:
...
func TestResourceReconcilerReconcile(t *testing.T) {
	...
	t.Run("acknowledge new types", func(t *testing.T) {
		...
		g.Expect(patchedResources).To(BeEmpty()) // Nothing should have been created
	...
	t.Run("create resources with acknowledged types", func(t *testing.T) {
		...
		g.Expect(patchedResources).To(Equal(map[string]struct{}{
			"rg1":    {},
			"rg2":    {},
			"secret": {},
		}))
	...
	t.Run("pause several resources", func(t *testing.T) {
		...
		r := &ResourceReconciler{
			Client: &FakeClient{
				Client: c,
				patchFunc: func(ctx context.Context, o client.Object, p client.Patch, po ...client.PatchOption) error {
					if o.GetObjectKind().GroupVersionKind().Kind == "ResourceGroup" {
						g.Expect(o.GetAnnotations()).To(HaveKeyWithValue(annotations.ReconcilePolicy, string(annotations.ReconcilePolicySkip)))
					} else {
						g.Expect(o.GetAnnotations()).NotTo(HaveKey(annotations.ReconcilePolicy))
					}
					if err := c.Get(ctx, client.ObjectKeyFromObject(o), o.DeepCopyObject().(client.Object)); err != nil {
		...
		g.Expect(r.Pause(ctx)).To(Succeed())
		g.Expect(patchedRGs).To(ConsistOf("rg1", "rg2", "secret"))
	})
}
