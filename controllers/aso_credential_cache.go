...
func (c *asoCredentialCache) clientOptsForASOResource(ctx context.Context, obj client.Object) (azcore.ClientOptions, error) {
	...
	if len(secret.Data[config.AzureAuthorityHost]) > 0 {
		authorityHost = string(secret.Data[config.AzureAuthorityHost])
	}

	armEndpoint := cloud.AzurePublic.Services[cloud.ResourceManager].Endpoint
	if len(secret.Data[config.ResourceManagerEndpoint]) > 0 {
		armEndpoint = string(secret.Data[config.ResourceManagerEndpoint])
	}

	armAudience := cloud.AzurePublic.Services[cloud.ResourceManager].Audience
	if len(secret.Data[config.ResourceManagerAudience]) > 0 {
		armAudience = string(secret.Data[config.ResourceManagerAudience])
	}
