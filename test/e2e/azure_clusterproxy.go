...
func (acp *AzureClusterProxy) collectActivityLogs(ctx context.Context, namespace, name, aboveMachinesPath string) {
	...
	case infrav1expalpha.AzureASOClusterKind:
		...
		err = clusterClient.List(ctx, rgs, client.InNamespace(cluster.Namespace))
		if err != nil {
			Logf("Error fetching activity logs for cluster %s in namespace %s.  Failed to list ASO ResourceGroups in namespace %s: %v", name, namespace, cluster.Namespace, err)
			return
		}
