...
| `CLUSTER_NAME`             | Name of an existing workload cluster.  Must be set to run specs against existing workload cluster. Use in conjunction with `SKIP_CREATE_MGMT_CLUSTER`, `GINKGO_FOCUS`/`GINKGO_LABEL_FILTER`, `CLUSTER_NAMESPACE` and `KUBECONFIG`. Filter tests with `GINKGO_FOCUS` (regex) or `GINKGO_LABEL_FILTER` (label expressions). |
| `CLUSTER_NAMESPACE`        | Namespace of an existing workload cluster.  Must be set to run specs against existing workload cluster. Use in conjunction with `SKIP_CREATE_MGMT_CLUSTER`, `GINKGO_FOCUS`/`GINKGO_LABEL_FILTER`, `CLUSTER_NAME` and `KUBECONFIG`. Filter tests with `GINKGO_FOCUS` (regex) or `GINKGO_LABEL_FILTER` (label expressions). |
