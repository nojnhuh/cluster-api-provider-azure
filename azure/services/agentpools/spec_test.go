...
func TestParameters(t *testing.T) {
	t.Run("no existing agent pool", func(t *testing.T) {
		...
		spec := &AgentPoolSpec{
			...
			KubeletConfig:          &KubeletConfig{CPUManagerPolicy: ptr.To("cpu manager policy")},
			KubeletDiskType:        ptr.To(infrav1.KubeletDiskType("kubelet disk type")),
			AdditionalTags:         map[string]string{"additional": "tags"},
			LinuxOSConfig:          &infrav1.LinuxOSConfig{Sysctls: &infrav1.SysctlConfig{FsNrOpen: ptr.To(6)}},
